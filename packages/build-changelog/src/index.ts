...
function parseBreakingChangeNote(message: string): string | null {
    ...
    for (const block of footerBlocks) {
        ...
        const footerMatch = /^(BREAKING CHANGE|BREAKING-CHANGE):\s*(.+)$/.exec(headerLine);
        if (!footerMatch) {
            continue;
        }
...
function parseCommitRecord(record: string): ChangelogEntry {
    const [
        hash, committedAtValue, date, rawMessage,
    ] = record.split(`\x1f`);
    ...
    const conventionalCommitMatch = /^([A-Za-z][A-Za-z0-9-]*)(?:\(([^)]+)\))?(!)?:\s*(.+)$/.exec(header);
